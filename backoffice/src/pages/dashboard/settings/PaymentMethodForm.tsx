...
const PaymentMethodForm = () => {
  ...
  const { register, handleSubmit, formState: { errors }, reset, watch } = useForm<PaymentMethodFormData>({
    defaultValues: {
      title: '',
      charge: '0',
      isActive: true,
  ...
  const loadMethod = async (methodId: string) => {
    try {
      const data = await settingsApi.getPaymentMethodById(methodId);
      reset({
        title: data.title,
  ...
  const onSubmit = async (data: PaymentMethodFormData) => {
    ...
    try {
      ...
      if (id) {
        await settingsApi.updatePaymentMethod(id, formData);
      } else {
        await settingsApi.createPaymentMethod(formData);
      }
      navigate('/settings/payment-methods');
    } catch (error) {
  ...
  return (
    <div className="max-w-2xl mx-auto relative z-10">
      <div className="flex items-center gap-4 mb-8">
        <Link to="/settings/payment-methods" className="p-2 hover:bg-gray-100 rounded-lg transition-colors">
          <ArrowLeft size={20} />
        </Link>
        <div>
          <h1 className="text-2xl font-bold text-gray-900">{id ? 'Edit Payment Method' : 'New Payment Method'}</h1>
          <p className="text-gray-500">Configure transaction options</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 space-y-6">
        ...
        <div>
          ...
          <textarea 
            ...
            rows={3}
            className="w-full px-4 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-black/5 text-gray-900"
            placeholder="Instructions or details for payment..."
          />
        </div>

        {/* Image Upload */}
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Badge/Logo</label>
        ...
        <div className="pt-6 border-t border-gray-100 flex justify-end gap-3">
          <Link to="/settings/payment-methods" className="px-6 py-2 border border-gray-200 text-gray-700 font-medium rounded-lg hover:bg-gray-50">
            Cancel
          </Link>
          <button 
            type="submit" 
