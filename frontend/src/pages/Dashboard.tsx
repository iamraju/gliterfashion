...
import { User, Package, MapPin, Heart, ArrowRight, ShoppingBag } from 'lucide-react';
...
const Dashboard = () => {
  ...
  const stats = [
    ...
    { label: 'Store Credit', value: formatCurrency(0), icon: ShoppingBag, color: 'bg-green-50 text-green-600' },
  ];
            ...
            <div className="lg:col-span-9 space-y-10">
              ...
              <div className="bg-white p-10 rounded-[48px] shadow-2xl shadow-gray-200/30 border border-gray-100">
                <div className="flex justify-between items-center mb-10">
                  ...
                  <Link to="/profile/orders" className="text-xs font-bold uppercase tracking-widest text-accent hover:underline flex items-center gap-2">
                 ...
                 <div className="bg-white p-8 rounded-[40px] shadow-xl shadow-gray-200/30 border border-gray-100">
                    ...
                    {user?.addresses && user.addresses.length > 0 ? (
                      <div className="space-y-4">
                        ...
                        <Link to="/profile/addresses" className="inline-block text-xs font-bold uppercase tracking-widest text-accent mt-4">Manage Addresses</Link>
                      </div>
                    ) : (
                      <div className="space-y-4">
                        <p className="text-sm text-gray-400">No addresses saved yet.</p>
                        <Link to="/profile/addresses" className="bg-gray-50 text-gray-900 px-6 py-3 rounded-xl text-xs font-bold uppercase tracking-widest inline-block hover:bg-gray-100 transition-all">Add Address</Link>
                      </div>
                    )}
                 </div>
              </div>
