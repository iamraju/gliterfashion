...
class AttributesService {
    async findAll() {
        return client_1.default.attribute.findMany({
            include: {
                values: true
            },
            orderBy: {
    ...
    async findById(id) {
        const attribute = await client_1.default.attribute.findUnique({
            where: { id },
            include: {
                values: true
            }
    ...
    async create(data) {
        const existingAttribute = await client_1.default.attribute.findUnique({ where: { slug: data.slug } });
        if (existingAttribute) {
            throw new Error('Attribute with this slug already exists');
        }
        return client_1.default.attribute.create({
            data: {
                name: data.name,
                slug: data.slug,
                values: {
    ...
    async update(id, data) {
        const existingAttribute = await client_1.default.attribute.findUnique({ where: { id } });
        if (!existingAttribute) {
            throw new Error('Attribute not found');
        }
        // Handle slug uniqueness
        if (data.slug) {
            const slugCheck = await client_1.default.attribute.findUnique({ where: { slug: data.slug } });
    ...
    async delete(id) {
        const attribute = await client_1.default.attribute.findUnique({ where: { id } });
        if (!attribute) {
            throw new Error('Attribute not found');
        }
        // Delete - will fail if used in variants via ProductVariantAttribute due to FK
        await client_1.default.attribute.delete({ where: { id } });
