...
class CouponsService {
    ...
    async findByCode(code) {
        const coupon = await client_1.default.coupon.findUnique({ where: { code } });
        if (!coupon) {
    ...
    async create(data) {
        const existingCoupon = await client_1.default.coupon.findUnique({ where: { code: data.code } });
        if (existingCoupon) {
        ...
        return client_1.default.coupon.create({
            data: {
    ...
    async update(id, data) {
        const existingCoupon = await client_1.default.coupon.findUnique({ where: { id } });
        if (!existingCoupon) {
            throw new Error('Coupon not found');
        }
        if (data.code) {
            const codeCheck = await client_1.default.coupon.findUnique({ where: { code: data.code } });
        ...
        if (data.maxDiscountAmount !== undefined)
            updateData.maxDiscountAmount = data.maxDiscountAmount || null;
    ...
    async delete(id) {
        const coupon = await client_1.default.coupon.findUnique({ where: { id } });
        if (!coupon) {
            throw new Error('Coupon not found');
        }
        ...
        try {
            await client_1.default.coupon.delete({ where: { id } });
        }
        catch (e) {
