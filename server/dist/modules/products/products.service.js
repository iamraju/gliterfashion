...
class ProductsService {
    ...
    async create(data) {
        ...
        const existingSku = await client_1.default.product.findUnique({ where: { sku: data.sku } });
        if (existingSku) {
    ...
    async update(id, data) {
        const existingProduct = await client_1.default.product.findUnique({ where: { id } });
        if (!existingProduct) {
            throw new Error('Product not found');
        }
        // Sanitize
